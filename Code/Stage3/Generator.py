...
class Contour2ToothGenerator_FaceColor_LightColor():
    ...
    def predict_interactive(self, data, whiteness=1.0, alignment_strength=1.0):
        """
        Interactive predict với các tham số tùy chỉnh:
        
        Parameters
        ----------
        whiteness          : float (0.0-2.0) – Scale cường độ face_light_color_bar.
                             >1 = răng sáng/trắng hơn, <1 = tối/tự nhiên hơn
        alignment_strength : float (0.0-2.0) – Scale cường độ teeth_contour trong conditioning.
                             >1 = contour mạnh hơn (răng nét), <1 = mềm hơn (tự do hơn)
        """
        self.netG.eval()
        ...
        with torch.no_grad():
            ...
            # ── Xử lý data giống Mask2TeethData_Process nhưng có scaling ────
            teeth_contour_rgb = cv2.cvtColor(teeth_contour_align, cv2.COLOR_BGR2RGB)
            mouth_rgb = cv2.cvtColor(mouth, cv2.COLOR_BGR2RGB)
            mask_np = np.array(mask)[:,:,0] / 255
            face_rgb = cv2.cvtColor(face, cv2.COLOR_BGR2RGB)

            # Light color extraction
            mouth_gray = cv2.cvtColor(mouth_rgb, cv2.COLOR_RGB2GRAY)
            clahe = cv2.createCLAHE(clipLimit=5.0, tileGridSize=(20,20))
            mouth_gray = clahe.apply(mouth_gray)
            _, light_mask = cv2.threshold(mouth_gray, 240, 255, cv2.THRESH_BINARY)
            light_color = np.uint8(mouth_rgb * np.expand_dims(light_mask/255, -1))

            face_color = np.uint8(face_rgb * (1. - np.expand_dims(mask_np, -1)))
            face_light_color = light_color + face_color

            # ── WHITENESS: scale face_light_color trước transform ───────────
            # whiteness > 1 → sáng hơn (điểm sáng mạnh hơn, skin tone giữ nguyên)
            # whiteness < 1 → tối hơn
            face_light_color_scaled = np.clip(
                face_light_color.astype(np.float32) * whiteness,
                0, 255
            ).astype(np.uint8)

            teeth_contour_t = self.transform(teeth_contour_rgb)
            face_light_color_bar = self.transform(face_light_color_scaled)

            # ── ALIGNMENT STRENGTH: scale teeth_contour trong conditioning ──
            # alignment_strength > 1 → contour rõ nét hơn, model theo sát cấu trúc
            # alignment_strength < 1 → contour yếu, model tự do generate
            teeth_contour_scaled = teeth_contour_t * alignment_strength

            mask_t = torch.from_numpy(mask_np).unsqueeze(0).float()
            noisy_image = (1. - mask_t)*teeth_contour_scaled + mask_t*torch.randn_like(teeth_contour_t)
            cond_image = torch.cat([teeth_contour_scaled, face_light_color_bar, noisy_image], dim=0)

            self.set_input({
                'bf_image': teeth_contour_t.unsqueeze(0),
                'cond_image': cond_image.unsqueeze(0),
                'mask': mask_t.unsqueeze(0),
                'mask_image': (teeth_contour_t*(1. - mask_t) + mask_t).unsqueeze(0),
            })

            self.output, self.visuals = self.netG.restoration(
                self.cond_image, y_t=torch.randn_like(self.bf_image),
                y_0=self.bf_image, mask=self.mask, sample_num=1)
            prediction = torch.from_numpy(
                self.visuals[-1].detach().float().cpu().numpy()[::-1, ...].copy())

            cond_display = cv2.cvtColor(face_light_color_scaled, cv2.COLOR_RGB2BGR)
            return prediction, cond_display
